...
## Другий white-label тенант — школа (2026-07-14)

Тенант **id=3 «Мовна школа»** (@language_schoolbot), **`is_school=True`** — режим
школи (кілька викладачів + групи + вкладка «Школа»), кольори `#2563EB/#0EA5E9`,
власник = Vova (469478065). Додано прапор **`--is-school`** у `add_tenant.py` +
параметр `is_school` у `create_tenant`. Бот налаштовано (команди/меню/опис),
redeploy → мультибот підхопив (3 боти: WordSnap + Марта + школа). E2E: підпис бота
школи → tenant 3. Далі: власник тисне Start → виставити `role='owner'` (SQL) →

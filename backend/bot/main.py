...
from scheduler.demo_conversion import demo_conversion_loop
...
async def cmd_start(message: Message):
    ...
    # Website attribution: лендинг wordsnap.app веде на `t.me/<bot>?start=<tag>`,
    # де tag = `web` (органіка) або `ad_<utm_campaign>` (платний трафік — щоб
    # відділити відкриття з конкретної реклами від органічних). Фіксуємо джерело
    # у `users.acquisition_payload` (first-touch) — так рахуємо РЕАЛЬНІ відкриття
    # додатку з сайту (воронка клік→відкриття, клік ловить Vercel Analytics).
    # Пишемо ДО демо/owner-гілки нижче (вона робить ранній return), не змінюючи
    # жодного онбординг-флоу — це лише мітка атрибуції.
    if payload and payload.startswith(("web", "ad_")) and not user.acquisition_payload:
